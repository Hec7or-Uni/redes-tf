...
void freewindow(uint32_t next) {
	if (((next<=numseqfirst) && (numseqfirst<(uint32_t)(numseqfirst+totalelems-getfreespace()))) // next menor
		...
		fprintf(stderr,"freewindow: intentando liberar un número de secuencia (%d) no almacenado en la ventana de emisión [%d,%d]\n",next,numseqfirst,numseqfirst+totalelems-getfreespace());
		exit(3);
	} else { // ok
